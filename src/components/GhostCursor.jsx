...
const MAX_SAFE_TRAIL_LENGTH = 32
...
function GhostCursor({
  ...
  const FilmGrainShader = useMemo(
    () => ({
      ...
      fragmentShader: `
        ...
        void main(){
          vec4 colorTex = texture2D(tDiffuse, vUv);
          float n = hash1(vUv.x*1000.0 + vUv.y*2000.0 + iTime) * 2.0 - 1.0;
          colorTex.rgb += n * intensity * colorTex.rgb;
  ...
  useEffect(() => {
    ...
    const trackingSurface = host?.closest('.sections-fx-wrap') ?? parent
    if (!host || !parent || !trackingSurface) return
    ...
    const maxTrail = Math.min(MAX_SAFE_TRAIL_LENGTH, Math.max(1, Math.floor(trailLength)))
    ...
    const animate = () => {
      ...
      const count = trailBufRef.current.length
      headRef.current = (headRef.current + 1) % count
      trailBufRef.current[headRef.current].copy(mat.uniforms.iMouse.value)
      const arr = mat.uniforms.iPrevMouse.value
      for (let i = 0; i < count; i += 1) {
        const srcIdx = (headRef.current - i + count) % count
      ...
      if (!hasPaintedRef.current) {
        const programCount = rendererRef.current?.info?.programs?.length ?? 0
        if (programCount > 0) {
    ...
    const onPointerMove = (event) => {
      const rect = trackingSurface.getBoundingClientRect()
      const inBounds =
      ...
      pointerActiveRef.current = inBounds
      ...
      if (inBounds) {
        ...
        host.style.setProperty('--ghost-x', `${x * 100}%`)
        host.style.setProperty('--ghost-y', `${(1 - y) * 100}%`)
    ...
    if (debugMode) {
      ...
      host.style.setProperty('--ghost-x', '50%')
      host.style.setProperty('--ghost-y', '50%')
  ...
  return (
    <div ref={containerRef} className={`ghost-cursor ${className ?? ''}`} style={mergedStyle}>
      {showFallback && <div className="ghost-cursor-fallback" />}
    </div>
  )
}
